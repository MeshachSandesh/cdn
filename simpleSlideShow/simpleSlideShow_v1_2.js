 ...
 *   autoScroll: boolean optional (default: true) - enable auto scroll
 *   autoScrollDelay: number (ms) optional (default: 3000) - delay between auto slides
 * }
 *
 *
...
function createSlideShow(options = {}) {
  ...
  const autoScroll = options.autoScroll !== false; // default to true unless explicitly set to false
  const autoScrollDelay =
    typeof options.autoScrollDelay === "number"
  ...
  function startAutoScroll() {
    if (!autoScroll || totalSlides <= 1 || autoScrollPaused) return;
    stopAutoScroll(); // Clear any existing interval
    autoScrollInterval = setInterval(() => {
      if (!isAnimating && !autoScrollPaused) {
        next();
      }
    }, autoScrollDelay);
  }
  ...
  // Pause auto scroll when the slideshow is not in the viewport
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          // console.log("Element went out of the viewport ❌", entry.target);
          pauseAutoScroll();
        }
      });
    },
